  ...
  return (
    <div className="flex leading-none gap-2">
      {pathnames.map((value, index) => {
        ...
        return last ? (
          <span key={to} className="text-2xl font-medium text-primary_color_1" aria-current="page">
          ...
          <Link
            to={to}
            key={to}
            className="text-primary_gray_2 flex items-center gap-2"
          >
            <span key={to} className="text-2xl font-light">
              {PathNamesTranslations[value]}
            </span>
            <span className="/">/</span>
          </Link>
